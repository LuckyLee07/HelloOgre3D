...
#if defined _WIN32
    const auto RelationProcessorDieCompat = LOGICAL_PROCESSOR_RELATIONSHIP( 5 );
#  if defined TRACY_WIN32_NO_DESKTOP
...
#  endif
    ...
    DWORD dsz = 0;
    _GetLogicalProcessorInformationEx( RelationProcessorDieCompat, nullptr, &dsz );
    if( GetLastError() == ERROR_INSUFFICIENT_BUFFER )
    {
        dieInfo = (SYSTEM_LOGICAL_PROCESSOR_INFORMATION_EX*)tracy_malloc( dsz );
        auto res = _GetLogicalProcessorInformationEx( RelationProcessorDieCompat, dieInfo, &dsz );
        assert( res );
    }
    else
    {
    ...
    {
        assert( ptr->Relationship == RelationProcessorDieCompat );
        // FIXME account for GroupCount
        auto mask = ptr->Processor.GroupMask[0].Mask;
        int core = 0;
        while( mask != 0 )
