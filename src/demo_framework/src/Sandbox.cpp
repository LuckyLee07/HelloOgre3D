...
UserInterfaceComponent* Sandbox::CreateUIComponent(size_t layerIndex)
{
...
Ogre::Camera* Sandbox::GetCamera()
{
    return camera_;
}

Ogre::Vector3 Sandbox::GetCameraForward()
{
    return camera_->getDerivedDirection();
}

Ogre::Vector3 Sandbox::GetCameraLeft()
{
    return -1.0f * camera_->getDerivedRight();
}

Ogre::Quaternion Sandbox::GetCameraOrientation()
{
    return camera_->getDerivedOrientation();
}

const Ogre::Vector3& Sandbox::GetCameraPosition()
{
    return camera_->getDerivedPosition();
}

Ogre::Vector3 Sandbox::GetCameraUp()
{
    return camera_->getDerivedUp();
}

bool Sandbox::GetDrawPhysicsWorld()
{
...
InfluenceMap* Sandbox::GetInfluenceMap()
{
    return influenceMap_;
}

InfluenceMapDrawer* Sandbox::GetInfluenceMapDrawer()
{
    return influenceMapDrawer_;
}

lua_State* Sandbox::GetLuaVM()
{
    return luaVM_;
}
...
Ogre::SceneManager* Sandbox::GetSceneManager()
{
...
Ogre::Real Sandbox::GetTimeInMillis()
{
    return Ogre::Real(simulationTime_);
}

Ogre::Real Sandbox::GetTimeInSeconds()
{
    return GetTimeInMillis() / 1000.0f;
}

UserInterface* Sandbox::GetUserInterface()
{
...
void Sandbox::SetDrawInfluenceMap(const bool drawInfluenceMap)
{
    if (influenceMapDrawer_)
    {
...
void Sandbox::SetMarkupColor(const int index, const Ogre::ColourValue& color)
{
