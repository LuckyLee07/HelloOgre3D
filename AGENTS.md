...
`HelloOgre3D` 的身份已从早期“教学 sample 沙盒”切换为 **“数据驱动的沙盒玩法运行时 + 编辑器”**：目标是大世界、多生物、可配置 AI、生物触发器、UGC 玩法，内容是**可作者编辑、可序列化、可热重载的数据**，而非代码。完整定调与优先级以 `docs/project-direction.md`（方向总纲 / 北极星）为准。

核心主轴：**把内容从代码变成数据**（定义 + 序列化 + 运行时实例化 + 热重载）。多生物、触发器、AI 可视化配置、UGC 都是这条脊梁的出口。

判断基准：
- 把“内容数据化”地基往前推的（CSV/Def 体系、统一事件总线、解耦去全局单例）→ 升优先级。
- 把“内容继续代码化 / 加新 UI 控件”的 → 降优先级。

默认工作原则：
- sample 不废弃，定位为运行时与编辑器的**验证场 / 回归面**；改动应保持现有 sample 可运行，但 sample 不再是终点。
- 优先做局部、可验证、可回退的小改动。
- 引擎耦合逻辑收口到 `runtime`；玩法规则逻辑数据化、Lua 化。
- 结构性改动应服务于解耦、数据驱动、便于后续 AI / 触发器 / 编辑器 / UGC 扩展。
