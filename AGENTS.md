...
`HelloOgre3D` 的当前身份重新收敛为 **“AI 学习与实验沙盒”**：目标是通过可运行、可观察、可逐步扩展的 sandbox 学习游戏 AI，而不是优先建设生产级数据驱动玩法运行时、编辑器或 UGC 平台。完整定调与优先级以 `docs/project-direction.md`（方向总纲 / 北极星）为准。

核心主轴：**把 AI 能力讲清楚、跑起来、看得见**。近期围绕感知、记忆、Blackboard、行为树 / 决策树 / FSM、团队通信、影响力地图、战术层、AI trace 与性能观测递进。
...
默认工作原则：
- sample 是 AI 学习章节、实验场和回归面；改动应保持现有 sample 可运行，并尽量让某个 sample 更清楚地展示一个 AI 概念。
- 优先做局部、可验证、可回退的小改动。
- 引擎耦合逻辑收口到 `runtime`；AI 热点逻辑优先放 C++，Lua 侧负责 sample 编排、行为配置和可读性。
- 结构性改动应服务于解耦、AI 可观测性、可复现实验和后续感知 / 记忆 / 战术扩展。
