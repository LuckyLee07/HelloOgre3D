...
> ⚠️ 文档状态（2026-06-11）：**历史/补充**。本文成文于早期非 AI 主线整理阶段，其优先级（尤其 FGUI-first、数据驱动切片、编辑器/UGC 相关排期）**已被 `docs/project-direction.md`（北极星）和 `docs/planning/long-term-iteration-plan.md` 取代**。当前项目阶段是 **AI 学习与实验沙盒**，长期目标是生产级游戏项目 / 玩法运行时。本文中 UI、runtime、调试、构建、稳定性等具体条目仍有参考价值，但**排序一律以北极星和长期迭代规划为准**。
>
> 目标：记录 `HelloOgre3D` 在 AI 之外值得持续迭代的方向，方便后续按优先级拆任务、补文档、做回归。
>
> 边界：AI 相关路线独立维护在 `docs/planning/ai-roadmap.md`；本文关注 UI、runtime、玩法基础设施、调试观测、构建测试、稳定性、对象系统等非 AI 主线。
...
| P2 | 对象系统组件化深化 | 降低新增对象类型的复制粘贴成本 | `docs/design/cpp-object-model-refactor-roadmap.md` |
...
- [x] 至少有一个非 FGUI 的自测入口（当前由 `tools/run_sandbox_smoke.ps1` 覆盖默认 `Sandbox8`，可通过 `HELLO_SANDBOX_SAMPLE` / `-Sample` 选择 `Sandbox1`-`Sandbox8`）。
...
详细对象模型、阶段边界、偏航检查和 `code-master` 参考取舍见 `docs/design/cpp-object-model-refactor-roadmap.md`。
...
这些是纯结构整理项，与对象模型重构（`docs/design/cpp-object-model-refactor-roadmap.md`）平行，按需穿插。每条执行前先 grep 当前代码复核（这些条目记录于早期，部分前置项可能已变）。统一遵守双轨过渡：迁移文件先留 forwarding header，确认引用迁完再删。
...
> 本节是早期 UI 优先阶段的旧排期，**已失效**。下面内容仅作历史记录；实际排期以 `docs/planning/long-term-iteration-plan.md` 和 `docs/planning/ai-technical-iteration-plan.md` 为准。

以下保留原文，仅作历史记录。专项细节如需参考，仍见 `docs/fgui/fairygui-business-framework-todo.md`、`docs/planning/ai-roadmap.md` 和对应实现文档。
...
- 补 texture、material、render command、triangle、draw/batch、clip/stencil、材质/纹理切换等更细的统计入口。
- 将 package、view、binding、timer、cache、资源引用计数继续接入 dump 或 Tracy 观测链路。
- 补服务层统计和 UI Tracy counter，便于对比不同 UI 规模下的开销。（基础 open/kind/peak、create/close/fail、Toast/Loading 元数据、C++ plot 桥、frame counters 和 visible pressure 实测报告已完成）
...
- Tab 焦点顺序。（已具备第一版）
- Windows IME commit、原生组合态统计、候选窗定位和清理链路已具备。
- 每个 layer 的默认输入穿透、sorting order、ESC 关闭参与规则。（已具备第一版）
- 打开 UI 时指定 parent/root。（已具备第一版）
- safe area 配置字段占位。（已具备第一版）

验收：

- 页面、弹窗、Toast、Guide、Top 层同屏时，输入命中、穿透和关闭顺序符合固定规则；当前由 `LayerBoundary` 自测覆盖核心规则。
- 文本输入至少覆盖英文、删除、提交、焦点切换和 IME 最小链路；当前已覆盖 Unicode commit、原生组合态统计、候选窗定位和组合态清理。
...
- 子页面 / 子组件基础挂载已完成，支持一个大 UI 拆成多个业务子模块；后续可继续补复杂子组件模板和 AutoGen 生成约定。
...
- 非 FGUI smoke test / selftest 已具备第一版：`tools/run_sandbox_smoke.ps1` 可拉起指定 sample、写入本次 run id、跳过默认 FGUI 示例 UI、抓取本次日志并扫描常见失败模式。
...
- 至少一个非 FGUI sample 可以脚本化验证；当前默认验证 `Sandbox8`，并支持切换 `Sandbox1`-`Sandbox8`。
...
- [x] FGUI texture / material / render command / triangle 明细统计（已有 source/alias 计数、texture 尺寸、同路径动态 loader 复用、draw/batch、clip/stencil 和材质/纹理切换统计）。
- [x] FGUI 服务层统计和 Tracy counter（基础服务统计、Lua perf dump、C++ Tracy plot 桥、frame render counters 和 visible pressure 实测报告已完成，见 `docs/perf/fgui-tracy-sample-20260523-213540-visible.md`）。
- [x] FGUI 键盘、滚轮、文本输入、焦点补齐已具备第一版，IME commit、Windows 原生组合态统计、候选窗定位和组合态清理已接入。
- [x] FGUI Progress / Slider / ComboBox / Transition / 虚拟列表补齐。
- [x] FGUI UI debug panel 第一版（已显示 UI/layer/binding/package/event/resource/render/perf/service，并接入 DebugPanelSelfTest / All 自测）。
- [ ] Runtime 中 Ogre / OIS / FGUI helper 收口。
  - [x] `runtime/diagnostics/RuntimeResourceDiagnostics` 已承接资源 dump。
  - [x] `runtime/profiling/RuntimeProfileCounters` 已承接 FGUI 服务层 counter 和 AI scheduler counter。
  - [x] 收口清单已明确下一批候选，当前继续以小步迁移方式推进，不单独做大重构。
  - [ ] 后续迁移候选：OIS/FGUI 输入桥、GameManager FGUI facade 拆薄、debug draw / nav debug mesh runtime 适配。
- [x] Object / Component inspector 第一版（`ObjectManager:buildObjectDebugSummary` 输出对象、组件、位置、生命状态和 AI driver 摘要）。
- [x] Resource dump 第一版（`GameManager:buildRuntimeResourceDump` 经 `runtime/diagnostics` 汇总 Ogre mesh/material/texture/skeleton/program 和 resource group 文件统计）。
- [ ] SkillTimeline 第一版。
- [ ] TriggerVolume 第一版。
- [x] ConfigManager 第一版（固定 seed、agent 数、队伍人数、出生模式、AI scheduler 参数、runtime diagnostic 上限）。
- [x] Sample preset 第一版（`default` 和 `ai_perf_smoke`，已接入 `Sandbox6` / `Sandbox7` / `Sandbox8` 与 smoke 参数）。
- [x] 非 FGUI 自测入口（`tools/run_sandbox_smoke.ps1` + `HELLO_SANDBOX_SAMPLE`）。
- [x] Smoke test（拉起进程、写入 run id、截取本次日志、扫描启动 / Lua / OGRE / 自测失败模式）。
