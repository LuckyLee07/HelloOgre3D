...
>
> 配套文档：设计与里程碑见 `docs/archive/visual-editor-implementation-plan.md`；为什么这么做见 `docs/reference/reference-minigame-patterns.md`。本文把里程碑拆成可认领的细任务，带**真实接口签名**（标注 [已存在]/[新增]/[修改]），签名指向 **HelloOgre3D 本仓库** 现有代码。
>
> 重要前提（已核实本仓库代码）：
