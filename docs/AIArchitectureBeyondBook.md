...
| TeamBlackboard first slice | Stage 3/4 交界已有 `Sandbox12`：复用动态战场，把同队共享敌人位置收敛到 Lua `TeamBlackboard`，并用 `SupportResponded` 验证队友响应 |
| InfluenceMap tactical slice | Stage 4 已有 `Sandbox13`：danger/support 双层影响图 + debug 绘制 + 低危险支援点选择 |
...
| T-03 | InfluenceMap 多层升级 | Tactic | ☑ | Stage 4：`Sandbox13` Lua first slice |
