...
>
> 使用方式：做架构决策时先读本文对应章节，再回看本项目现状（`docs/architecture-improvement-plan.md` / `docs/cpp-object-model-refactor-roadmap.md`）。本文结论涉及对方实现细节处均带证据（文档名或 `文件:行`），但这些证据指向的是 **MiniGame_Dev 仓库**，不是本仓库。
>
...
> 这次调研印证了 `docs/architecture-improvement-plan.md` 的方向（解耦 + 组件化 + 数据驱动），但**纠正一处**：不要把“带反射的统一节点”当成所有对象的脊梁。多生物运行时走 CSV 原型（轨道 A）；反射 + flag 多通道留给编辑器/可视化配置层（轨道 B）。
