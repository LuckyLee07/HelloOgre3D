...
- **BT trace sampling / tick LOD / frame budget / condition cache（2026-06-20）**：`BehaviorTreeDriver:SetDebugTraceSampleInterval(interval)` / `GetDebugTraceSampleInterval()` 已导出给 Lua；默认也可用环境变量 `HELLO_BT_TRACE_SAMPLE_INTERVAL` 控制。`SetRuntimeTickIntervalMs(intervalMs)` / `GetRuntimeTickIntervalMs()` 与 `SetRuntimeTickStaggerEnabled(enabled)` / `IsRuntimeTickStaggerEnabled()` 已导出；`SetRuntimeMaxTreeTicksPerFrame(maxTicksPerFrame)` / `GetRuntimeMaxTreeTicksPerFrame()` 已导出；`ConfigManager` 可从 preset `behaviorTree.tickIntervalMs` / `tickStagger` / `maxTreeTicksPerFrame` 或 `HELLO_BT_TICK_INTERVAL_MS` / `HELLO_BT_TICK_STAGGER` / `HELLO_BT_MAX_TREE_TICKS_PER_FRAME` 配置第一版 BT runtime tick LOD 与每帧树 tick budget。`Blackboard:GetRevision()` / `GetKeyRevision(key)` 与 `LuaCondition:SetResultCacheEnabled(...)` / `SetResultCacheTtlMs(...)` / `AddResultCacheDependencyKey(...)` / cache stats 已导出，用于 `BehaviorTreeLoader` 配置黑板依赖条件缓存。
...
- **TeamBlackboard typed facts（2026-06-20）**：`TeamBlackboardService:rememberTeamFact(...)` / `writeBestTeamFactToBlackboard(...)` / `getTeamBlackboardTypedFactCount()` / `getTeamBlackboardTypedReportCount()` 已导出给 Lua；`TeamBlackboard.lua` 用它同步 `SupportRequested` / `SupportResponded` / `FocusTarget` / `RetreatPoint` / `FormationSlot`。
...
- **Tactics debug config（2026-06-20）**：`TacticalService:setTacticalInfluenceLayerDebugOrder(layerName, drawOrder)` 已手术式补入绑定；现有 `drawTacticalInfluenceLayer` / `rebuildTacticalInfluenceLayerDebugVisual` 会在 C++ `TacticalDebugDrawService` 记录 per-layer y/threshold/maxCells/drawNeutral/projectToNav/nav/order 配置，并在 `buildTacticalInfluenceDebugSummary()` 输出 `[TacticalDebugDraw]`。
...
- 改 .pkg/绑定后：编译 + 跑用到该绑定的 sample（DT/BT 跑 `Sandbox7/8`；影响图跑 `Sandbox17/18`；`SandboxPerception` 跑 `hearing_danger`）。
