...
- 三轮定稿首轮：双机连续分面主壳与实体炮罩、降低护栏/箭头亮度、冷暖光照与无碰撞渐变舞台；30静态几何和角色固定盒碰撞保持。实现与对应三关/查询/原生证据见[三轮定稿](../dev-design/plans/2026-09-19-crossfire-final-three-rounds.md)。

- 三轮定稿收口：战术提示限制在左上 388×50 区域，阶段/行动按语义分两行，角色名称与落点编号避让；结算只保留品牌/地区和报告内可操作入口，不再留失效执行按钮。结果/菜单隐藏世界指挥线与警戒圈；标题直接说明规划/执行/自动开火，下一关按钮点明地区。
...
- Crossfire 命令先只读校验再提交；暂停计划整批通过后才同刻写入并启动时钟。攻击入口和执行时均核对存活、可见、18 m 射程；拒绝保留旧计划和暂停，E 不误启动。错误提示按 UI 时钟保留 4 s，其他反馈 2 s，菜单内冻结；卡片区分新计划与暂停旧动作。胜负确认只清一次在途弹和原生粒子，28 槽 Lua 结果特效继续短收尾。生产闭包回归为 `tools/test_crossfire_commands.lua`。
...
- 三关表现统一：冷却筒/碟塔分开结构、涂层与金属材料；冷却/联锁核心只换材料，复用原几何、碰撞和状态环位置。后两关背墙横牌/核心小牌均为无碰撞平面，使用宽实线设施符号，与战术路径区别。选关突出 07/08/09 编号；结算逐项显示 `fullTeam/withinDamage/withinTime` 的达成状态，缺值明确未记录；第三关胜利的次要右按钮为 H 帮助，Enter 返回选关。见[三关品质实施](../dev-design/plans/2026-09-19-crossfire-three-court-quality.md)。
