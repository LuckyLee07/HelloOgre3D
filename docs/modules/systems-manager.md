...
- 暂停原生枪火粒子由 `ObjectLifecycleSystem::SetTransientParticlesPaused` 处理，ObjectManager 仅转发临时 context。只遍历已注册子弹与延迟清理 scene node，沿子树设置 Ogre speed factor 0/1；当前三种模板默认速度为 1，不是任意粒子的通用速度恢复器。不缓存节点指针，不暂停全局渲染 Controller，不新增 Lua 导出。
