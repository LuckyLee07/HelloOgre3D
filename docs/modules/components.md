...
- **Controller 互斥**：一个 agent 同时只能有一个主动 driver；`ai_soldier` 装 `AIController`，`player_soldier` / `commander_soldier` 装 `PlayerController`，两者都实现 `IAgentController`，不通过新建 `PlayerObject` 区分控制来源。`commander_soldier` 不装 `WeaponComponent`；PlayerController 的射击/换弹路径必须把武器缺失视为合法组合并安全 no-op。
