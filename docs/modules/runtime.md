...
- Apple 普通窗口路径当前在 `ClientManager::ConfigureRenderSystemForApple` 中固定选择 1280×720；它不读取仅供 Windows 后台窗口使用的 `HELLO_WINDOW_WIDTH/HEIGHT`。macOS 的动态窗口和 1080p 验收必须先扩展这一平台路径，不能只设置环境变量。
