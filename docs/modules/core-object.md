...
| `SandboxServices.h` | 注入聚合 | `{objects, physics, input, sandbox, script, objectFactory, navigation, raycast, scene, scriptService}`（P1 核心） |
...
- **P1（已解决）**：SandboxServices 已落地下发并通知组件，`tools/check_sandbox_architecture.ps1` 已加静态门禁；Physics/Weapon/AI/Locomotion/FSM/AgentObject/BlockObject/SoldierObject 热点已移除 `g_*`/`GameManager` 兜底，Render/Anim/Input include 已清，Raycast/Scene/Script 等门面已由应用层注入为 `RaycastService` / `SceneService` / `ScriptService`，SceneFactory root node / UIManager camera / CameraService / InputManager 依赖已由应用层注入，NavigationMesh/ObjectManager 场景访问已走 SceneFactory，manager 层 `g_ObjectManager`/`g_SandboxMgr` 已删除。
- 事件总线目前同步直发，无缓冲队列/节流。
