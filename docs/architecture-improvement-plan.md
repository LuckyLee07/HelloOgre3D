...
> - 对象模型分阶段路线、术语定义见 `docs/cpp-object-model-refactor-roadmap.md`（本文是它的“证据 + 可执行”补充）。
> - 非 AI 主线优先级见 `docs/project-roadmap.md`。
> - AI 能力路线见 `docs/ai-roadmap.md`。
>
...
- **C5 修正文档/代码漂移**：`cpp-object-model-refactor-roadmap.md` 多处引用 `GameObject` / `VehicleObject`，但代码实际继承链为 `SandboxObject → BaseObject → AgentObject → SoldierObject`（无这两个类）。同步更新该文档术语与现状。
...
- [ ] P1 SandboxServices 注入，去 sandbox→game 反向依赖与 `g_*` 热点
- [ ] P2 停止对象层转发样板，Lua/sample 走组件直取
- [ ] P3 拆薄 GameManager FGUI 转发
- [ ] P4 组件容器 `unique_ptr` 化，清理 `SAFE_DELETE`/双重持有
- [ ] P5 组件 owner 统一 `BaseObject`，解除子类依赖
- [ ] P6 字符串 key → 类型化访问 + 常量头
- [ ] P7 `PerceptionQuery` + driver 工厂
- [ ] C1 删 `UIService` 空壳
- [ ] C2 `SandboxMgr` 减肥
- [ ] C3 `ObjectManager` registry / update 拆分
- [ ] C4 `IComponent` update order 显式化
- [ ] C5 修正 `cpp-object-model-refactor-roadmap.md` 术语漂移
