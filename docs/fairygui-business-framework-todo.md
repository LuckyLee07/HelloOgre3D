...
- [~] 渲染裁剪与遮罩已有矩形 scissor/stencil 状态桥接、MaskProbe/GraphMaskProbe 样例和 `GGraph/DrawNode` 最小绘制链路，但还不是完整 GPU 像素级 stencil。
...
- [x] MaskProbe / GraphMaskProbe 真实 mask 可视化样例。
...
- [x] 增加 `HELLO_FGUI_SELF_TEST_ALL=1` 一键 FGUI 自测入口，集中跑 Act37、Act38、Layer、Mask、GraphMask、Input、Lifecycle、BusinessFlow、Cleanup。
- [x] 增加 `HELLO_FGUI_LONG_LOOP_SELF_TEST=1` 长循环自测入口，循环打开关闭 Act37、Act38、Layer、Mask、GraphMask、TextInput 并检查资源/生命周期回零。
