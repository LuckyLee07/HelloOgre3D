...
2. 输入与层级边界：Tab 焦点顺序、layer 输入穿透 / ESC / sorting order 规则、指定 parent/root 打开 UI 和 safe area 第一版已完成；下一步补 Windows IME 最小链路和更细输入命中可视化。
...
- [~] Tab 焦点顺序已具备第一版并接入 LayerBoundary 自测，IME 输入链路待补。
