...
| Chapter 7 Knowledge sample | Stage 3 已有 `Sandbox9`：Lua `KnowledgeSource` 限频评估，结果写入 C++ `Blackboard`，BT 只消费黑板数据 |
...
| Chapter 8 Vision/Memory vertical slice | Stage 3 已有 `Sandbox10`：用可读面板、视野圈、lastKnown 标记和 BT action 展示 `VisionSensor -> MemoryStore -> BT` |
| Chapter 8 Perception communication sample | Stage 3 已有 `Sandbox11`：6 个 agent 随机分散出生、交替分队、随机移动/追击/射击，用 EnemySighted 队内广播与 TeamMemory shared enemy 复刻原 Chapter 8 的感知通信主线 |
| TeamBlackboard first slice | Stage 3/4 交界已有 `Sandbox12`：复用动态战场，把同队共享敌人位置收敛到 Lua `TeamBlackboard`，并用 `SupportResponded` 和 scan/apply 调度统计验证队友响应 |
| InfluenceMap tactical slice | Stage 4 已有 `Sandbox13`：danger/support 双层影响图 + debug 绘制 + 低危险支援点选择，并通过 `updateIntervalMs` / scan/apply smoke stats 展示更新节奏 |
| HearingSensor / DangerSensor | Stage 3 已有 `Sandbox14` Lua-first 最小切片：用脚本化枪声 / 危险线写入 local blackboard，BT 在没有直接视线时调查声音或规避危险；后续再收口为 C++ `ISensor` |
| Formation / cooperative BT first slice | Stage 4 已有 `Sandbox15` Lua-first 最小切片：formation slot 分配 + `CallForBackup` / `WaitForSquadMate` / `MoveToFormationSlot` BT 分支，用 smoke 验证协作行为 |
...
| 能力 | 默认频率 / 预算 | 说明 |
|---|---|---|
| VisionSensor | 200ms / agent，按 agentId 错峰 | raycast 和 FOV 不每帧全量跑，`Sandbox10` preset 已默认启用 `AIScheduler` |
| HearingSensor | 事件驱动，100ms 合并窗口 | 高频声音事件先合并再写 bb |
| Memory decay | 500ms 或 1000ms | 只处理活跃 memory entry |
| TeamBlackboard sync | 200ms | `Sandbox12` / `Sandbox13` 已先以 sight pair budget + team apply budget 分批，不再每帧全 agent pair 扫描 |
| InfluenceMap layer | 500ms+，每帧 cell budget | `Sandbox13` 已先以 Lua `updateIntervalMs` 做降频与 updates/skips 观测，大地图阶段再补分片预算 |
...
| Influence smoke | 固定 seed sample | AI 避开危险区域或偏好掩体区域，layer 更新次数、跳过帧数和 cell writes 可见 |
| Hearing / Danger smoke | `tools\run_sandbox_smoke.ps1 -Preset hearing_danger` | 没有直接视线时，agent 可因声音进入调查，或因危险线进入规避，`heard` / `danger` / `investigations` 可见 |
| Formation smoke | `tools\run_sandbox_smoke.ps1 -Preset formation_tactics` | formation slot、请求支援、等待队友与协作移动至少各触发一次，`slots` / `backupCalls` / `waitAssignments` 可见 |
...
| S-03 | HearingSensor 最小 sample | Sensor | ☑ | Stage 3：`Sandbox14` Lua-first slice，C++ `ISensor` 化后置 |
| S-04 | DangerSensor 最小 sample | Sensor | ☑ | Stage 3：`Sandbox14` Lua-first slice，C++ `ISensor` 化后置 |
...
| P-04 | Chapter 8 Vision/Memory readable slice | Perception | ☑ | Stage 3 |
| P-05 | Chapter 8 Perception communication sample | Perception/Event | ☑ | Stage 3 |
| T-00 | TeamBlackboardService + team target sharing | Tactic | ☑ | Stage 3/4：`Sandbox12` Lua first slice |
| T-01 | Formation 系统 + Lua 配置 | Tactic | ☑ | Stage 4：`Sandbox15` Lua-first slice |
| T-02 | 协作 BT 节点（CallForBackup/WaitForSquadMate/MoveToFormationSlot/FollowSquadOrder）| Tactic | ☑ | Stage 4：`Sandbox15` 已覆盖前三个节点，FollowSquadOrder 后置 |
| T-03 | InfluenceMap 多层升级 | Tactic | ☑ | Stage 4：`Sandbox13` Lua first slice，已补 update interval 与 smoke stats |
