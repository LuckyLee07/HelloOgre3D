...
## 地表纵深、植被与接地追加（2026-09-12）

P3 门面与战斗反馈完成后，正常镜头仍能看到大面积单层地面、角色和建筑接地不足，以及已批准目标稿中的荒漠植被缺失。本轮继续收敛这三项，但把视觉与玩法真源严格分开。

- 新增无 Bullet 刚体的 `CreateVisualPlane`：对象仍由 `ObjectManager` 管理；`OBJ_TYPE_PLANE` 将它排除在 navmesh 固定几何之外，无刚体则避免进入物理射线和 AI 视线遮挡。GL3+ core profile 使用显式 GLSL 150 unlit-alpha shader，D3D9 预留 HLSL 2.0 对应实现。
- 九块低对比尘土/磨损层打散地板网格；六组掩体、场景设备、relay 主体和七个角色获得接触阴影。角色阴影复用固定槽位，只按真实 agent 脚点更新，死亡/空槽移到地下。
- 24 组项目专用荒漠草丛采用 crossed card，全部位于外围服务带与掩体肩部；alpha 裁切和低饱和 tint 在最终 GL 抓帧中去除了明显亮边。六组掩体从约 `x=±10` 内移到约 `x=±8`，让正常镜头能形成前中远景框架，同时保留约 10 米中央通道与西侧绕行。

![地表旧化、荒漠植被与角色接触阴影](dev-design/specs/assets/sandbox19-runtime/grounding-vegetation-720.png)

### 追加验证与边界

- macOS arm64 Release 构建通过；Apple M1 Pro / OpenGL 4.1、1280×720 真实窗口实际加载 `relay_overlay` program、`ground_dust.png`、`contact_shadow.png` 与 `desert_grass.png`。最终画面无黑色贴片、缺材质或 shader 报错。
