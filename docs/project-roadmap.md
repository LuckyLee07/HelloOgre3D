...
- 补服务层统计和 UI Tracy counter，便于对比不同 UI 规模下的开销。（基础 open/kind/peak、create/close/fail、Toast/Loading 元数据和 C++ plot 桥已完成，后续补 Tracy viewer 实测）
...
- Tab 焦点顺序。（已具备第一版）
- Windows IME commit 最小链路已具备，继续补原生组合态和候选窗。
...
- 文本输入至少覆盖英文、删除、提交、焦点切换和 IME 最小链路；当前已覆盖 Unicode commit，Windows 原生组合态和候选窗待补。
...
- [~] FGUI 服务层统计和 Tracy counter（基础服务统计、Lua perf dump、C++ Tracy plot 桥和 frame render counters 已完成，Tracy viewer 实测待补）。
- [~] FGUI 键盘、滚轮、文本输入、焦点补齐已具备第一版，IME commit 第一版已接入，Windows 原生组合态和候选窗待补。
