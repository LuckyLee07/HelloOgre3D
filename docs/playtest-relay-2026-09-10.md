...
场景复用 Nobiax 模块和既有士兵，增加浅暖墙面、低饱和绿色设备、入口/目标区域及中线标记。2026-09-11 根据“与目标效果差距仍大”的反馈继续加入分层外墙、窗片、横向低掩体、开放棚架和 relay 屋顶/桅杆，并将大面积安全区描边改为四角短标记。西侧路线具有真实碰撞墙与可达导航。相机现使用 6.5 米后距、3.2 米相对高度、0 米前视，WASD 沿相机平面移动，Q/E 转向、滚轮有限缩放。HUD 提供任务、两张队友卡、命令栏、目标和落点标记、暂停/声音设置、结算和重试；AI 观察器、路径与性能诊断默认隐藏。
...
实际画面见下方。概念稿中浅暖环境、绿色设备、青色友军、琥珀任务标识与深色紧凑 HUD 已落入游戏。2026-09-11 的 [视觉收敛复盘](sandbox19-visual-convergence-2026-09-11.md) 将此前过早的“已完成”拆为功能闭环和视觉完成度：当前视野结构、敌我可读性、HUD 层级与空间一致性已通过实机检查，材质/资产细节仍为 PARTIAL。当前仍采用现有低多边形资产、Ogre D3D9 和 Gorilla 字体/UI，没有替换角色模型、引入 PBR/后处理或新 FGUI 资源包。既有 Gorilla 可可靠呈现和命中测试，因此本轮沿用并把业务拆为局部模块；不宣称概念图是运行截图。

声音使用项目内生成的九个简短 PCM WAV，经 runtime 单通道后端播放；支持音量、静音、保存和停止。它提供选择、指令、暂停、结果、射击/受击反馈，不是空间混音系统。后台测试静音，API 与资源读取已验，实际扬声器听感未验。

![2026-09-11 实际 1080p 集火与战斗 HUD](dev-design/specs/assets/sandbox19-runtime/combat.png)

![2026-09-11 实际 720p 命令与落点](dev-design/specs/assets/sandbox19-runtime/orders-720.png)

![2026-09-10 自然通关结算；背景为视觉收敛前版本，仅保留玩法闭环证据](dev-design/specs/assets/sandbox19-runtime/victory.png)
