...
- [x] `Toast` 服务：已有短提示、自动关闭、排队和去重，并接入 `service skin` 样式注册与覆盖。
- [x] `Loading` 服务：已有 modal loading、引用计数和超时清理，并接入 `service skin` 样式注册与覆盖。
- [x] `MessageBox/Dialog` 服务：已有确认/取消按钮和回调，并接入 `service skin` 样式注册与覆盖。
- [x] `PopupMenu` 服务：已有列表项、外部关闭、目标跟随定位，并接入 `service skin` 样式注册与覆盖。
- [x] `Tip` 服务：已有指定坐标提示、hover 延迟、离开隐藏和锚点定位，并接入 `service skin` 样式注册与覆盖。
- [x] `GuideMask` 服务：已有遮罩、文案、多矩形高亮和点击穿透，并接入 `service skin` 样式注册与覆盖。
...
当前 FGUI 收口统一按 `docs/fgui/fairygui-production-gate.md` 执行。日常小改用 Fast，生命周期/资源/层级改动用 RuntimeOnly，合主或 C++ runtime 改动用 Full。
...
- [x] dump 指定 UI/package、关闭指定业务 UI 和 AI 专用视图已具备第一版，并由 `DebugPanelSelfTest` / `AiDebugPanelSelfTest` 覆盖；AI DebugPanel 已支持 `focusAgentId` / `filterText` 参数化筛选，交互式筛选控件仍可后续增强。
...
- [x] 新增 `docs/fgui/fairygui-business-ui-standard-flow.md`，固定“导出资源 -> 生成 manifest/AutoGen/registry -> 填 Ctrl/Model/View -> 补 selftest -> 静态检查”的业务 UI 接入流程。
...
## 2026-05-25 通用服务产品化补充

- [x] `PopupMenu` 增加 `anchorRect/followRect/targetRect` 定位策略，支持按目标矩形 `bottomLeft/topLeft/right/left/center` 跟随并自动限制在屏幕内。
- [x] `Tip` 增加 `ShowHoverTip/HideTip`，支持 hover 延迟显示、离开隐藏和锚点矩形定位。
- [x] `GuideMask` 支持 `highlightRects` 多矩形高亮，遮罩段按高亮区域切分，点击多个高亮区不会关闭，点击遮罩区仍按规则关闭。
- [x] Toast/Loading/MessageBox/PopupMenu/Tip/GuideMask 接入 `RegisterServiceSkin/GetServiceSkin/ResolveServiceSkin`，尺寸、字体、颜色、alpha 和动态资源描述从硬编码抽到统一样式入口。
- [ ] 后续如果接入真实 FairyGUI 通用控件包，还需要把 `service skin.resource` 映射到具体包体组件或图片资源。
