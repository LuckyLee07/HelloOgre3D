...
- 日期：2026-09-12。
- 生成方式：项目自制 [多边形生成器](../../../tools/generate_relay_zone_stencil.py)以 Pillow 绘制 A1 字形和透明 alpha，再按既有竖直 VisualPlane 的 UV 方向旋转；最终为 512×1024 RGBA PNG。没有外部字体、商标或第三方图像。
- 用途：Sandbox19 左翼实体墙面的区域喷涂标识；`Relay/ZoneStencil` 借用现有 GL/HLSL 透明叠加程序。画面层无刚体，不改变导航和 AI 视线；对应墙体仍有原有实体碰撞。
